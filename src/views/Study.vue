...
onMounted(async () => {
  try {
    ...
    index.value = answered.value.length // 从已答数接着考（刷新后自动恢复）
...
async function submit() {
  if (!canSubmit.value || result.value || submitting.value) return
  ...
  try {
    ...
    saveProgress() // 作答即存，后台异步
  } catch (e) {
    submitErr.value = true // 失败不卡死，按钮恢复可重试
  } finally {
    submitting.value = false
  }
}

// 复制拟态题：挖空原文 +（错误答案）+ 正确答案
async function copyOno() {
  const q = current.value
  if (!q || q.type !== 'ono') return
...
<template>
  <div>
    ...
    <div v-else-if="current">
      ...
      <p v-if="submitErr" class="msg-err" style="margin-bottom: 8px">
        提交失败，请重试
      </p>
      <button
        v-if="!result"
